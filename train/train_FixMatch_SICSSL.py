...
def main():
    ...
    parser.add_argument('--dataset', default='cifar10', type=str,
                        choices=['cifar10', 'cifar100', 'stl10', 'svhn', 'smallimagenet','cifar100_con','cifar10_con','stl10_con'],
                        help='dataset name')
    ...
    elif args.dataset == 'stl10' or args.dataset=='stl10_con':
        args.num_classes = 10
    ...
    if args.dataset == 'cifar10_con' or args.dataset == 'cifar100_con' or args.dataset == 'svhn_con':
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            mask = torch.eq(im_labels, im_labels.T).float()


            logits_mask = torch.scatter(
                torch.ones_like(mask),
                1,
                torch.arange(mask.size(0)).view(-1, 1).to(args.device),
                0
            )
            ...
            mask_pos_view = torch.zeros_like(mask).to(args.device)
            ...
            feature_logits /= args.T_con

            log_prob = F.normalize(feature_logits.exp(), dim=1, p=1).log()
            Scontrast = - torch.sum((mask_pos_view_class * log_prob).sum(1) / mask_pos_view.sum(1)) / mask_pos_view.shape[0]
            ...
            loss = loss + args.con_weights * (Scontrast + Lcontrast)

            loss.backward()
            losses.update(loss.item())
            losses_x.update(Lx.item())
            losses_u.update(Lu.item())
            losses_Scon.update(Scontrast.item() * args.con_weights)
        ...
        if args.local_rank in [-1, 0]:
            ...
            logger.info(f'train_loss : {losses.avg:.4f} | loss_x : {losses_x.avg:.4f} | loss_u : {losses_u.avg:.4f} | Lcontrast : {losses_Lcon.avg:.4f} | Scontrast : {losses_Scon.avg:.4f}')
    if args.local_rank in [-1, 0]:
        args.writer.close()
