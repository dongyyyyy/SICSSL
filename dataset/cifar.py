...
def get_cifar10(args, root):
    ...
    ir2=l_samples[-1]/np.array(l_samples)

    return train_labeled_dataset, train_unlabeled_dataset, test_dataset, ir2, l_samples , u_samples

def get_cifar10_contrastive(args, root):
    ...
    train_labeled_dataset = CIFAR10SSL(
        root, train_labeled_idxs, train=True,
        transform=TransformFixMatch_revision(mean=cifar10_mean, std=cifar10_std))
...
def get_cifar100(args, root):
    ...
    train_unlabeled_dataset = CIFAR100SSL(
        root, train_unlabeled_idxs, train_labeled_idxs, train=True,
        transform=TransformFixMatch_revision(mean=cifar100_mean, std=cifar100_std))
...
def get_cifar100_contrastive(args, root):
...
def get_stl10(args, root):
    ...
    return train_labeled_dataset, train_unlabeled_dataset, test_dataset, ir2, l_samples

def get_stl10_contrastive(args, root):
    ...
    train_labeled_dataset = datasets.STL10(root, split="train", transform=TransformFixMatchSTL(mean=stl10_mean, std=stl10_std), download=True)
    train_unlabeled_dataset = datasets.STL10(root, split="unlabeled",
                                                         transform=TransformFixMatchSTL(mean=stl10_mean, std=stl10_std),
                                                         download=True)
    test_dataset = datasets.STL10(root, split="test", transform=transform_val, download=True)
...
def get_svhn_con(args, root):
    ...
    train_labeled_dataset = SVHN_labeled(root, train_labeled_idxs, split='train', transform=TransformFixMatch(mean=SVHN_mean, std=SVHN_std))
    train_unlabeled_dataset = SVHN_unlabeled(root, train_unlabeled_idxs, train_labeled_idxs, split='train',
                                             transform=TransformFixMatch(mean=SVHN_mean, std=SVHN_std))
    test_dataset = SVHN_labeled(root, test_idxs, split='test', transform=transform_val, download=True)
    ir2=l_samples[-1]/np.array(l_samples)
...
def train_split(labels, n_labeled_per_class, n_unlabeled_per_class, args):
    ...
    for i in range(args.num_classes):
        ...
        if args.seed !=0:
            np.random.shuffle(idxs)#Since we brought the experimental results on CIFAR 100 presented in the CoSSL paper, we shuffle idxs for CIFAR-100 as like CoSSL.
...
def make_imb_data(max_num, class_num, gamma, flag = 1, flag_LT = 0):
    ...
    if flag == 0 and flag_LT == 1: # reverse
        class_num_list = list(reversed(class_num_list))
    elif flag == 0 and flag_LT == 2: # uniform
        return list([int(max_num) for i in range(class_num)])
    elif flag == 0 and flag_LT == 3: # random
...
class TransformFixMatch_revision(object):
    def __init__(self, mean, std, img_size=32):
        self.weak = transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.RandomCrop(size=img_size,
        ...
        self.strong = transforms.Compose([
            ...
            transforms.RandomCrop(size=img_size,
                                  ...
                                  padding_mode='reflect'),
            transforms.ToTensor(),])
        self.strong.transforms.insert(0,RandAugment(3,4))
        self.strong.transforms.append(CutoutDefault(16))
...
DATASET_GETTERS = {'cifar10': get_cifar10,
                    'cifar10_con':get_cifar10_contrastive,
                   'cifar100': get_cifar100,
                   'cifar100_con': get_cifar100_contrastive,
                   'stl10': get_stl10,
                   'stl10_con': get_stl10_contrastive,
                   'svhn': get_svhn,
